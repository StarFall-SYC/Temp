...
export const useNovelStore = create<NovelState>((set, _get) => ({
  novels: [],
  currentNovel: null,
  userNovels: [],
  isLoading: false,
  ...
  fetchUserNovels: async (username: string) => {
    ...
    try {
      const response = await axios.get<ApiResponse<Novel[]>>(
        `${API_BASE_URL}/novels/user/${username}`
      );
  ...
  fetchNovelDetail: async (username: string, title: string) => {
    ...
    try {
      const response = await axios.get<ApiResponse<Novel>>(
        `${API_BASE_URL}/novels/${username}/${encodeURIComponent(title)}`
      );
  ...
  updateNovel: async (username: string, title: string, updates) => {
    ...
    try {
      const response = await axios.put<ApiResponse<Novel>>(
        `${API_BASE_URL}/novels/${username}/${encodeURIComponent(title)}`,
        updates
      );
      ...
      if (response.data.success && response.data.data) {
        ...
        set((state) => ({
          ...
          currentNovel:
            state.currentNovel?.id === updatedNovel.id ? updatedNovel : state.currentNovel,
          isLoading: false,
        }));
  ...
  deleteNovel: async (username: string, title: string) => {
    ...
    try {
      const response = await axios.delete<ApiResponse>(
        `/novels/${username}/${encodeURIComponent(title)}`
      );
      ...
      if (response.data.success) {
        ...
        set((state) => ({
          novels: state.novels.filter(
            (novel) => !(novel.author === username && novel.title === title)
          ),
          userNovels: state.userNovels.filter(
            (novel) => !(novel.author === username && novel.title === title)
          ),
          currentNovel:
            state.currentNovel?.author === username && state.currentNovel?.title === title
              ? null
              : state.currentNovel,
          isLoading: false,
        }));
  ...
  addChapter: async (username: string, title: string, chapter) => {
    ...
    try {
      const response = await axios.post<ApiResponse<Chapter>>(
        `${API_BASE_URL}/novels/${username}/${encodeURIComponent(title)}/chapters`,
        chapter
      );
      ...
      if (response.data.success && response.data.data) {
        ...
        set((state) => {
          if (
            state.currentNovel &&
            state.currentNovel.author === username &&
            state.currentNovel.title === title
          ) {
            const updatedNovel = {
              ...state.currentNovel,
            ...
            return {
              ...
              userNovels: state.userNovels.map((n) =>
                n.id === updatedNovel.id ? updatedNovel : n
              ),
              isLoading: false,
            };
          }
  ...
  uploadCover: async (username: string, title: string, file: File) => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        set((state) => ({
          novels: state.novels.map((novel) =>
            novel.author === username && novel.title === title ? { ...novel, coverUrl } : novel
          ),
          userNovels: state.userNovels.map((novel) =>
            novel.author === username && novel.title === title ? { ...novel, coverUrl } : novel
          ),
  ...
  removeNovelFromWebSocket: (username: string, title: string) => {
    set((state) => ({
      novels: state.novels.filter((novel) => !(novel.author === username && novel.title === title)),
      userNovels: state.userNovels.filter(
        (novel) => !(novel.author === username && novel.title === title)
      ),
      currentNovel:
